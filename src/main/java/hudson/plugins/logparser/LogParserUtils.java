...
public final class LogParserUtils {
    ...
    public static String standardizeStatus(final String status) {
        ...
        // Arbitrary tag: this condition check is commented out so that
        // arbitrary status can come through
            ...
            if (!skipParsingRule(parsingRule)) {
                try {
                    ...
                    if (!LogParserConsts.LEGAL_STATUS.contains(tag.toUpperCase(Locale.ENGLISH))) {
                        if (!Arrays.asList("OK", "END", "WARN")
                                .contains(tag.toUpperCase(Locale.ENGLISH))) {
                            extraTags.add(tag);
                        }
                    }
